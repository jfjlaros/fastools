...
def fa_motif2bed(input_handle, output_handle, motif):
    ...
    """
    for record in SeqIO.parse(input_handle, 'fasta'):
        for m in fastools._find_motif(record, motif):
            output_handle.write(
...
def edit(input_handle, edits_handle, output_handle):
    ...
    :arg stream output_handle: Open writable handle to a FASTA file.
    """
    edits_records = fastools._edits_read(edits_handle)
...
def csv2fa2(input_handle, output_handles, skip_header=False):
    ...
    for record in reader:
        fastools._write_seq(output_handles[0], record[1], record[0])
        fastools._write_seq(output_handles[1], record[2], record[0])
